...
bool UOABackendManager::HasErrors(const TSharedPtr<FJsonObject>& JsonObject) const
{
	...
	const bool bHasSimpleError = JsonObject->HasTypedField<EJson::String>(TEXT("error"));

	if (bHasSimpleError)
	{
		const FString ErrorMessage = GetStringOr(TEXT("error"), TEXT("unknown error"));

		UE_LOG(LogTemp, Error, TEXT("Backend error: message= %s"), *ErrorMessage);
		return true;
	}
	...
	if (bHasFault)
	{
		...
		UE_LOG(LogTemp, Error, TEXT("Backend error: name= %s, fault= %s"), *ErrorName, *FaultType);
		return true;
	}
